...
### 3. Chapter File Structure (Per Book)

Books are stored as **per-chapter files** — not monoliths — to enable parallel agentic editing, clean diffs, and surgical PR reviews.

```
books/aadhaar-esign/
├── metadata.yaml              # Book metadata (title, subtitle, author, description, etc.)
├── chapter-index.yml          # Order of files to assemble
├── manuscript/
│   ├── 00-frontmatter.md      # Title, subtitle, compiled date, brief description
│   ├── ch01.md                # Chapter 1
│   ├── ch02.md                # Chapter 2
│   ├── ...
│   ├── ch13.md                # Chapter 13
│   ├── app-appendices.md      # Appendices (timeline, CA directory, stats, glossary)
│   ├── back-references.md     # Full URL references
│   ├── 90-about-this-book.md  # Context, audience, methodology, how to read
│   └── 91-about-the-author.md # Author bio
└── (outputs generated by CI)
```

**chapter-index.yml** (example):
```yaml
...
**Rules:**
- Every file in `manuscript/` is an independent, self-contained markdown document
- Files are numbered: `00-` prefix for frontmatter, `ch01`–`chNN` for chapters, `app-` for appendices, `back-` for backmatter, `90-`/`91-` for about sections
- `chapter-index.yml` is the single source of truth for assembly order
- The build script reads `chapter-index.yml`, concatenates files in order, then runs pandoc
- A monolithic `manuscript.md` is still supported as fallback but not preferred

### 4. Structural Requirements (Per Book)
...
| **About this book** | End matter | Context, audience, methodology, how to read |
| **About the author** | End matter | CashlessConsumer bio, lens, related work |
...
### 10. Quality Checklist (Pre-Merge)
